sap.ui.define([
    ...
    return BaseController.extend("cadastroequip_fornecedor.controller.Master", {
        formatter: formatter,
		...
		onOpenViewSettings: function (oEvent) {
			...
			if (!this._pViewSettingsDialog) {
				this._pViewSettingsDialog = Fragment.load({
					...
					name: "cadastroequip_fornecedor.view.ViewSettingsDialog",
					controller: this
		...
		onOpenViewCreate: function () {
			...
			if (!this._pViewCreateDialog) {
				...
				this._pViewCreateDialog = Fragment.load({
					id: this.getView().getId(),
					name: "cadastroequip_fornecedor.view.CreateEntry",
					controller: this
				}).then(function (oDialog) {
					// connect dialog to the root view of this component (models, lifecycle)
					this.getView().addDependent(oDialog);
		...
		onValueHelpRequestDriver: function () {
			...
			if (!this._oDialogSearchDriver) {
				this._oDialogSearchDriver = Fragment.load({
					id: oView.getId(),
					name: "cadastroequip_fornecedor.view.DriverSelect",
					controller: this
		...
		onPressUpload: function (oEvent) {
			...
			if (!this._oDialogEquipsUpload) {
				this._oDialogEquipsUpload = Fragment.load({
					id: oView.getId(),
					name: "cadastroequip_fornecedor.view.EquipsUpload",
					controller: this
		...
		handleUploadEquip: function (oEvent) {
			...
			const sServicePath = "sap/opu/odata/sap/ZGTWPM095_EQUIPAMENTO_SRV/E_AnexosSet";
